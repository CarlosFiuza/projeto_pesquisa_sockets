...
class Server:
    ...
    def run(self):
        try:
            print("Digite o IP, porta, num_max pessoas e identificador para criar uma sala:")
            while True:
                try:
                    host, port, num, id = input().split()
                    room = Room(host, int(port), int(num), id)
                    thread = threading.Thread(target = room.run, args = (lambda : self.server_up, ))
                    thread.start()
                    room_map = types.SimpleNamespace(id=id, thread=thread)
